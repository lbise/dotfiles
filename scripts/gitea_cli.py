...
class GiteaManager:
    ...
    def create_pr(
        ...
        try:
            ...
            if e.response.status_code == 422:
                ...
                print(
                    "Error: Authentication failed. Check your GITEA_TOKEN",
                    file=sys.stderr,
                )
            else:
                print(
    ...
    def _handle_http_error(self, e: requests.exceptions.HTTPError) -> None:
        ...
        if e.response.status_code == 404:
            ...
            print(
                "Error: Authentication failed. Check your GITEA_TOKEN", file=sys.stderr
            )
    ...
    def show_pr_files(
        ...
        try:
            ...
            print("-" * 80)
            print(
                f"Total: {len(files)} files, +{total_additions} additions, -{total_deletions} deletions"
            )
...
def main():
    ...
    pr_files_parser = subparsers.add_parser(
        "pr-files", help="Show files changed in a pull request"
    )
    ...
    pr_comment_parser = subparsers.add_parser(
        "pr-comment", help="Add a comment to a pull request"
    )
    ...
    pr_comments_parser = subparsers.add_parser(
        "pr-comments", help="List review comments on a pull request"
    )
