...
class ConfluenceClient:
    ...
    def update_page(self, page_id: str, title: str, body: str, new_version: int) -> dict:
    ...
    def create_page(
        ...
        """Create a new page in the given space, optionally under a parent."""
        url = f'{self.base_url}/rest/api/content'
        payload: dict = {
            'type': 'page',
            'title': title,
...
def parse_confluence_url(url: str) -> dict:
    ...
    # Try to parse Cloud format: /spaces/SPACE/pages/123456/Title
    cloud_match = re.match(r'/(?:wiki/)?spaces/([^/]+)/pages/(\d+)(?:/(.+))?', parsed.path)
    if cloud_match:
        result['space_key'] = cloud_match.group(1)
        result['page_id'] = cloud_match.group(2)
        if cloud_match.group(3):
            result['title'] = unquote(cloud_match.group(3).replace('+', ' '))
        return result
...
def _resolve_page_id(client: ConfluenceClient, url_or_id: str) -> str:
    """Resolve a URL or bare ID to a numeric page ID string."""
    if url_or_id.startswith('http://') or url_or_id.startswith('https://'):
        ...
        if 'page_id' in parsed:
            ...
            page = client.get_page_by_title(parsed['space_key'], parsed['title'])
            if not page:
                print(
                    f'Error: Page "{parsed["title"]}" not found in space {parsed["space_key"]}',
                    file=sys.stderr,
                )
                sys.exit(1)
            return page['id']
...
def update_page_cmd(
    ...
    try:
        page_id = _resolve_page_id(client, url_or_id)

        # Fetch current page to get version number and existing title
        page = client.get_page_by_id(page_id)
        current_version = page['version']['number']
        title = new_title if new_title else page['title']

        # Read new body from file or stdin
        if content_source == '-':
            body = sys.stdin.read()
        else:
        ...
        updated = client.update_page(page_id, title, body, current_version + 1)

        links = updated.get('_links', {})
        if 'webui' in links:
            page_url = client.base_url + links['webui']
        else:
            page_url = f'{client.base_url}/pages/viewpage.action?pageId={page_id}'

        print(f'Page updated successfully: {page_url}')
...
def create_page_cmd(
    ...
    try:
        parent_id: Optional[str] = None
        if parent:
            parent_id = _resolve_page_id(client, parent)
        ...
        created = client.create_page(space_key, title, body, parent_id=parent_id)

        links = created.get('_links', {})
        if 'webui' in links:
            page_url = client.base_url + links['webui']
        else:
            page_url = f'{client.base_url}/pages/viewpage.action?pageId={created["id"]}'
...
def get_email() -> Optional[str]:
    """Get email for Cloud authentication. Optional for on-premises."""
...
def main():
    ...
    update_parser.add_argument('url_or_id', help='Confluence URL or page ID')
    update_parser.add_argument(
        '--content',
        required=True,
        metavar='FILE',
        help='File containing new body in Confluence storage format (use - for stdin)',
    )
    update_parser.add_argument(
        '--title',
        dest='new_title',
        default=None,
        help='New page title (default: keep existing title)',
    )

    # Create page command
    create_parser = subparsers.add_parser('create', help='Create a new page')
    create_parser.add_argument('space_key', help='Space key (e.g. RNDMUR)')
    create_parser.add_argument('title', help='Title of the new page')
    create_parser.add_argument(
        '--content',
        required=True,
        metavar='FILE',
        help='File containing body in Confluence storage format (use - for stdin)',
    )
    create_parser.add_argument(
        '--parent',
        default=None,
        metavar='URL_OR_ID',
        help='URL or ID of the parent page (omit to create at space root)',
    )
    ...
    client = ConfluenceClient(confluence_url, token, email)
    ...
    if args.command == 'get':
        ...
        update_page_cmd(client, args.url_or_id, args.content, new_title=args.new_title)
    elif args.command == 'create':
        create_page_cmd(client, args.space_key, args.title, args.content, parent=args.parent)
