---
name: arconnect
description: Create or edit ARConnect inter-core messages
---
...
- **Section Header:**
  • Specifies the source (from) and destination (to) cores, the API name, and the starting message ID (`msg_base`).
...
All the necessary infrastructure code is generated by the andromeda build system when parsing the YAML file for both the source and destination core.

## Workflow

In order to implement a new ARConnect message the user must do the following operations:

* Define the new message in the appropriate YAML file.
