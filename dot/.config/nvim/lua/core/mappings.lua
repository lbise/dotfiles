...
local yank_path = function()
	...
	tools.copy_relative_path(full_path)
end
...
local allowed_prefixes = {
	vim.fn.expand("$HOME/gitrepo") .. "/",
}

local allowed_patterns = {
	-- only keep REAL glob cases here
	tools.glob_to_lua_pattern("**/*dmtx*/**"),
}

local function is_allowed(fname)
	-- Format everything when not at work
	...
	for _, prefix in ipairs(allowed_prefixes) do
		if vim.startswith(fname, prefix) then
			return true
		end
	end

	-- SLOW: pattern checks (only if needed)
...
-- Format on save but only certain files
vim.api.nvim_create_autocmd("BufWritePre", {
	...
	callback = function(args)
		...
		if not is_allowed(fname) then
			return
		end

		-- Run conform
		vim.notify("Format " .. fname)
